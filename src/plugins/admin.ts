...
export const create = buildCreate(({ name, type, logger, config }) => {
  ...
  registerCommand(name, 'admin', ['private', 'group'], ({ event, args, back }) => {
    if (!isAdmin(event.userId)) return;
    const qq = getNumberByMsg(args);
    if (qq === null) {
      //list
      const msg: string[] = Array.from(admins!).map((x, i) => `${i + 1}. ${x}`);
      msg.unshift('管理员列表: ');
      back(msg);
  ...
  return {
    ...
    getHelp() {
      return [`${cmdTag}admin 列出管理员列表`, `${cmdTag}admin [qq] 添加/移除管理员权限`];
    },
    noClose: true,
  };
});
