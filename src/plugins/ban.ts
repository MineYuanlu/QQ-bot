...
export const create = buildCreate(({ name, type, logger }) => {
  ...
  registerCommand(
    ...
    "unban",
    ["private", "group"],
    async ({ args, back, type, event }) => {
      if (!isAdmin(event.userId)) return;
      const qq = getNumberByMsg(args);
      if (qq === null)
        return back(
          "请输入QQ号" + (type === "private" ? "" : ", 或直接at对方")
        );
      else if (typeof qq === "string") return back(`无效的QQ号: [${qq}]`);
  ...
  registerCommand(
    ...
    async ({ bot, args, back, type, event }) => {
      ...
      if (type === "group" && group === null)
        group = (event as GroupMessageEvent).groupId;
