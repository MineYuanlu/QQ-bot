...
import { exitHook } from './lib/tools/procexit';
...
const loader = async (
  ...
  try {
    ...
    const plugin: { create?: CreateHandler } = await import('./plugins/' + config.plugin);
    if (!plugin.create)
      console.error(prefix.ERROR, '无法加载插件:', Logger.pluginColor(name), '因为其没有构造器!');
    else {
      const conf = config.config || {};
      const bot = toArray(config.bot, (x): x is number => !!x && !isNaN(x)).map((x) => Number(x));
      const service = toArray(config.service, isServiceString);

      await plugin.create(name, config.plugin, conf, bot, service);
    }
  } catch (err) {
    console.error(prefix.ERROR, '无法加载插件:', Logger.pluginColor(name), '加载时出错:', err);
  }
};
...
(async () => {
  //bot白名单
  const bots = ((arr) => {
    if (arr && (arr = arr.map((x) => Number(x)).filter((x) => !isNaN(x))).length)
      return new Set(arr);
  ...
  if (bots) {
    ...
    registerInternal('handleConnect', async (bot) => {
      ...
      try {
        if (!disconnected.has(id))
          console.warn(prefix.WARN, '机器人列表外的机器人加入:', Logger.qqColor(id, 'B'));
        disconnected.add(id);
      } catch (err) {
        console.error(err);
      } finally {
    ...
    registerInternal('handleDisconnect', (bot) => bots.has(bot.botId) || handlerAction.prevent);
  }
  ...
  //插件启动
  await Promise.all(Object.keys(config.plugins).map((name) => setPluginEnable(name, true)));
  ...
  exitHook(async () => {
    await Promise.all(Object.keys(config.plugins).map((name) => setPluginEnable(name, false)));
  });
})();
